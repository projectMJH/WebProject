...
public class DataBoardDAO {
	...
	public List<DataBoardVO> databoardListData(int page)
	{
		...
		{
			getConnection();
			String sql="SELECT no,name,subject,dbday,hit,num "
					+"FROM (SELECT no,name,subject,dbday,hit,rownum as num "
					+"FROM (SELECT no,name,subject,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday,hit "
	...
	public DataBoardVO databoardDetail(int no)
	{
		...
		try
		{
			...
			sql="SELECT no,name,subject,content,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday,hit,filename,filesize "
				+ "FROM jspDataBoard "
				+ "WHERE no="+no;
			ps=conn.prepareStatement(sql);
			ResultSet rs=ps.executeQuery();
