...
public class MusicList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<GenieMusicVO> list=dao.musicListData(curpage);
		...
		if (startPage>1)
		{
			out.println("<li><a href=\"MainServlet?mode=21&page="+(startPage-1)+"\">&lt;</a></li>");
		}
		for(int i=startPage;i<=endPage;i++)
		{
			if(i==curpage)
			{
				out.println("<li class=\"active\"><a href=\"MainServlet?mode=21&page="+i+"\">"+i+"</a></li>");
			}
			else
			{
				out.println("<li><a href=\"MainServlet?mode=21&page="+i+"\">"+i+"</a></li>");
			}
		}
		if (endPage<totalpage)
		{
			out.println("<li><a href=\"MainServlet?mode=21&page="+(endPage+1)+"\">&gt;</a></li>");
		}
		out.println("</ul>");
		out.println("</div>");
		...
		List<GenieMusicVO> cList=new ArrayList<GenieMusicVO>();
		...
		if(cookies!=null)
		{
			for(int i=cookies.length-1;i>=0;i--)
			{
				if(cookies[i].getName().startsWith("music_"))
				{
					...
					GenieMusicVO vo=dao.musicCookieData(Integer.parseInt(mno));
					cList.add(vo);
				}
			}
		}
		...
		{
			GenieMusicVO cvo=cList.get(i);
			if(i>8) break;
			out.println("<a href=MusicBeforeDetail?mno="+cvo.getMno()+">");
//			out.println("<a href=MainServlet?mode=22&mno="+cvo.getMno()+">");
