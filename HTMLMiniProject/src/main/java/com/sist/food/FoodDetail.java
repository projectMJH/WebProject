...
public class FoodDetail extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 				= String[] getParameterValue() => 다중값
		 * 					=> checkbox
		 * 				= encoding(byte[]) 전송 => decoding(원상 복구)
		...
		out.println("<h3>"+vo.getName()+"&nbsp;<span style=\"color:orange\">"+vo.getScore()+"</span>");
		out.println("</td>");
		out.println("</tr>");
		...
		// 화면 분할 (전체 컬럼이 12등분 되어 있다 -> 세로분할 시 합이 12가 되도록)
