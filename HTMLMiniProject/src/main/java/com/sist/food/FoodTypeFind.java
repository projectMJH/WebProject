...
public class FoodTypeFind extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(startPage>1)
		{
			out.println("<li><a href=\"MainServlet?mode=3&page="+(startPage-1)+"&type="+type+"\">&lt;</a></li>");
		}
		for(int i=startPage;i<=endPage;i++)
		{
			if(i==curpage)
			{
				out.println("<li class=active><a href=\"MainServlet?mode=3&page="+i+"&type="+type+"\">"+i+"</a></li>");
			}
			else
			{
				out.println("<li><a href=\"MainServlet?mode=3&page="+i+"&type="+type+"\">"+i+"</a></li>");
			}
		}
		if(endPage<totalpage)
		{
			out.println("<li><a href=\"MainServlet?mode=3&page="+(endPage+1)+"&type="+type+"\">&gt;</a></li>");
		}
		out.println("</ul>");
		out.println("</div>");
