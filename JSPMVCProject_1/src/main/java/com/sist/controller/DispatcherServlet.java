...
// DOM(Document Object Model) => XML을 트리형태로 메모리에 저장 후 데이터 읽기 (CRUD)
// SAX(Simple API for XML) => 태그를 한개씩 읽어서 데이터 추출
// 문제점
// => AWS에 호스팅 => 리눅스 (우분투) => 경로명의 문제 \\ <-> /
// => 작성 => 리눅스에서 호환 (path를 읽어오는 방식으로는 안됨)
@WebServlet("*.do")
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try
		{
			...
			// => MVC 구조에서는 Controller를 유지보수 => 공사중
			for(int i=0; i<list.getLength(); i++)
			{
				...
				// 클래스 정보 읽기 => 메모리 할당 / 메서드 호출 / 멤버 변수 주입....
